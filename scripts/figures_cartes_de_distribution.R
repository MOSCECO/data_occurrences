...
# Données filtré par le seuil arbitraire de 0 spécimens par espèces

path_figures_carte_pas <- here("figures", "cartes_de_distribution_pa")
...
# dataframe of all shared species of more than 0 specimens ----
TBC <- do.call(
  rbind, 
  lapply(
    names(pa), 
...
figures_cartes_distributions <- lapply(
  ...
  function(bn) {
    ...
    f <- list.files(
      path_figures_carte_pas_isl_tax, pattern = gsub(" ", "-", bn)
    )
    if (f %>% length() == 0) {
